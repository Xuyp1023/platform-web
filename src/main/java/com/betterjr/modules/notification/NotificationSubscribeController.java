...
public class NotificationSubscribeController {
    ...
    public @ResponseBody String querySubscribeByCustNo(final HttpServletRequest request, final Long custNo,
            final int flag, final int pageNum, final int pageSize) {
        logger.debug("消息订阅列表-查询 入参:custNo=" + custNo);
        return exec(() -> subscribeService.webQuerySubscribeByCustNo(custNo, flag, pageNum, pageSize), "消息订阅列表-查询 出错",
                logger);
    }
    ...
    public @ResponseBody String cancelSubscribe(final Long custNo, final Long sourceCustNo, final String profileName,
            final String channel) {
        logger.debug("消息订阅列表-查询 入参:custNo=" + custNo + " sourceCustNo=" + sourceCustNo + " profileName=" + profileName
                + " channel=" + channel);
        return exec(() -> subscribeService.webCancelSubscribe(custNo, sourceCustNo, profileName, channel), "取消订阅  出错",
                logger);
    }
    ...
    public @ResponseBody String confirmSubscribe(final Long custNo, final Long sourceCustNo, final String profileName,
            final String channel) {
        logger.debug("消息订阅列表-查询 入参:custNo=" + custNo + " sourceCustNo=" + sourceCustNo + " profileName=" + profileName
                + " channel=" + channel);
        return exec(() -> subscribeService.webConfirmSubscribe(custNo, sourceCustNo, profileName, channel), "订阅  出错",
                logger);
    }
}
